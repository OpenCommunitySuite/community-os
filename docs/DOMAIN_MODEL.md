...
У Membership может быть несколько исторически значимых оснований, заявлений, решений или иных актов признания. Наличие нескольких таких оснований у одного Subject не создаёт автоматически несколько одновременных Membership или несколько Voting Rights. Несколько самостоятельных Membership relation instances одного Subject в одном Community допустимы только если их отдельная identity и lifecycle следуют из конкретной community/legal semantics; количество заявлений само по себе этого не доказывает.
...
Для электронного подписания различаются **semantic target** — Document Revision/Representation в предметном смысле — и **exact cryptographic target** — конкретные electronic data, криптографически покрытые подписью. Exact cryptographic target может быть Representation либо technical artifact/data binding; technical file/artifact не становится Representation автоматически. Подписание одних electronic data не распространяется автоматически на другую Representation той же Revision.

Electronic signature artifact/provider result не является Signing автоматически. Он проходит validation/mapping, Subject/target interpretation и domain admissibility согласно ADR-010/011, после чего контекст «Документы и формализация» признаёт Signing либо rejection. Cryptographic validity не создаёт Domain Power и не доказывает допустимость действия от имени другого Subject/органа.

Для recognized electronic Signing должны быть исторически объяснимы exact signed content, signer/Subject mapping, authority/admissibility basis, применимые версии signing rule/policy и validation/trust context там, где их изменение могло повлиять на recognition. Revalidation не создаёт новое Signing и не переписывает молча первоначальный validation/recognition context. Отдельные universal `Electronic Signing`, `Signing Evidence` и `Signature Validation` entities не вводятся.

Несколько Subjects либо один Subject в разные исторически различимые моменты могут иметь несколько самостоятельных Signing facts по одному semantic target, если applicable policy признаёт их отдельными действиями. Производный статус комплектности подписей может существовать как Read Model / Projection, но не заменяет individual Signing facts.
