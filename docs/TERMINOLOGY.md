...
При замене физический Meter сохраняет собственную identity; история старого прибора не переносится в новый. Один Meter может последовательно использоваться в разных Accounting Points, если это допускает предметная ситуация.

Серийный номер, внешний device ID и номер пломбы могут быть значимыми идентификационными либо доказательственными сведениями, но не являются универсальными глобальными ключами Meter identity и не создают replacement автоматически.
...
**Разовое начисление** — первичное начисление, инициируемое по отдельному предметному основанию и не предполагающее само по себе периодического повторения.

**Внецикловое начисление** — первичное начисление, выполняемое вне обычного регулярного момента расчёта; оно может использовать те же rules, tariff semantics и historical period, что и regular accrual.

Разовый/внецикловый характер является process/timing semantics обычного начисления и не создаёт отдельный фундаментальный тип Accrual. Он не является correction, recalculation, cancellation или storno уже подтверждённого начисления.

**Перерасчёт начисления (Accrual Recalculation)** — новое исторически значимое применение applicable rules к historical context уже подтверждённого Accrual. Исходный расчёт сохраняется. Последовательный перерасчёт должен позволять определить как original Accrual, так и непосредственно предшествующий effective result/change, который он изменяет.

**Отмена финансового эффекта начисления (Accrual Cancellation)** — прослеживаемое прекращение действующего financial effect подтверждённого Accrual на достаточном основании без удаления исходного начисления. Отмена не является Refund, Storno или negative Accrual.

Если subsequent correction/recalculation/cancellation несколько, их упорядоченная связь является provenance requirement; универсальные Accrual Revision, Correction Chain или state machine не вводятся.
...
**Переплата** — признанное финансовое состояние, при котором ранее применённая к исполнению сумма в текущем эффективном состоянии финансовых отношений оказывается избыточной согласно применимой предметной семантике. Само арифметическое превышение суммы платежа над текущими обязательствами или задолженностью не образует переплату автоматически.

**Избыточная ранее применённая сумма (excess applied amount)** — производное процессное значение: часть суммы Payment, ранее применённая к исполнению Financial Obligation, которая после уменьшения или отмены текущего effective obligation больше не требуется для его исполнения. Это не самостоятельная фундаментальная сущность и не новый вид Payment Allocation.

Excess applied amount не становится автоматически переплатой, авансом или нераспределённым остатком. До отдельного допустимого disposition/Reallocation соответствующая часть Payment продолжает учитываться в current effective financial use и не может быть повторно использована Initial Allocation. Если excess получает предметный смысл Overpayment, это уже признанное финансовое состояние, а не чисто арифметическое следствие.
...
Нераспределённый остаток, переплата, excess applied amount и аванс не тождественны. Существенное назначение аванса сохраняется, а последующее изменение финансового смысла excess определяется соответствующим financial process.
...
Идентичность платежа относится к предметному Payment финансового контекста и не определяется идентичностью или количеством банковских транзакций, строк выписки, кассовых документов либо иных внешних представлений.

Если исправление подтверждает, что речь остаётся о том же предметном Payment, его identity может сохраняться при исторически прослеживаемом исправлении характеристик.

**Инвалидация признания платежа** (`Payment recognition invalidation`) — специализированный результат процесса исправления ошибочного recognition, при котором исходное признание и его provenance сохраняются, но Payment более не имеет действующего финансового эффекта, поскольку само recognition признано ошибочным. Invalidation признания платежа не является возвратом денежных средств, source/external cancellation или универсальным статусом жизненного цикла Payment.

Если ошибочное recognition неверно определило количество предметных Payments, replacement Payments получают самостоятельные identities согласно применимому процессу признания; история ошибочных recognitions сохраняется.
...
**Возврат денежных средств (Refund)** — специализированный финансовый процесс/смысл, при котором существует предметно достаточное основание вернуть средства, происхождение которых прослеживается к одному или нескольким ранее состоявшимся движениям денежных средств либо к финансовому состоянию, возникшему из таких движений. Фактическое исполнение Refund является новым Payment, а не изменением первоначального Payment.

**Financial Obligation to return** — контекстное использование обычного Financial Obligation в Refund-процессе, а не отдельный тип обязательства. Оно определяет обязанность вернуть сумму на установленном refund basis и исполняется обычным Payment Allocation.

**Refundable amount** и **committed refundable amount** в BP-FIN-003 являются вычисляемыми процессными величинами, а не самостоятельными сохраняемыми предметными состояниями или сущностями. Committed refundable amount не является Payment Allocation, Reserve или техническим Reservation.

Refund не требует универсальной связи 1:1 с конкретным original Payment, но произвольное новое обязательство на выплату без возвратного происхождения не является Refund.
...
Юридическая форма сама по себе не определяет owning relation Community OS. В частности, ГПХ может быть формой уже существующего служебного/трудового отношения либо основанием самостоятельного внешнего Contractual Relationship — в зависимости от реального предметного смысла отношения. Тип лица и название документа не являются универсальным критерием.
...
**Расход сообщества (Expense)** — самостоятельный финансово-управленческий факт конкретного использования либо предметно признанного предполагаемого использования средств сообщества для определённой цели или деятельности.

Recognized Expense имеет собственную identity и достаточно определимые сумму и денежную единицу. Его identity определяется coherent use/intended-use case и не выводится автоматически из документа, Financial Obligation, Payment, Bank Transaction, Cash Disbursement, Budget Item, Funding Source, Meter или Accounting Point.
...
Expense не тождествен Financial Obligation, Payment или Payment Allocation и не определяет бухгалтерское либо налоговое признание. Financial Obligation, исходящий Payment, supplier document, Community Consumption, resource loss, Budget plan, Management Decision или Contractual Relationship не создают Expense автоматически.

Expense может быть связан с одним или несколькими Financial Obligations и Payments; обратная cardinality также не является универсально 1:1. Если связь несёт monetary scope, она должна быть объяснима и не создавать второй Payment Allocation effect.

Expense может существовать без Budget Item, до Payment и, при явно применимой financial-management semantics, до отдельно признанного Financial Obligation. Generic plan/proposal/estimate не является Expense.

Факт последующего возмещения стоимости собственниками или другими сторонами через Accrual/Financial Obligation не отменяет Expense автоматически и сам по себе не определяет, является ли соответствующая стоимость Expense.

**Распределение расхода по статьям сметы (Expense Budget Distribution)** — исторически значимая специализированная relation/action, связывающая monetary portions признанного Expense с одной или несколькими Budget Items для план-факт классификации.
...
Источник финансирования не тождествен статье начисления, финансовому обязательству, платежу, банковской транзакции или направлению использования средств.
...
- заёмные средства;
- другие применимые источники средств.
...
**Финансирование расхода** — предметная связь конкретного расхода с одним или несколькими источниками финансирования, отражающая его фактическое финансово-управленческое покрытие.

Один источник может финансировать несколько расходов, а один расход — покрываться несколькими источниками.

При additive Expense Financing total effective financing не превышает Expense amount, а одна monetary portion Expense не учитывается как фактически покрытая несколькими Funding Sources одновременно. Expense может оставаться частично или полностью без признанного Expense Financing.

Источник финансирования, направление использования средств, Expense Budget Distribution и финансирование расхода являются различными понятиями.

Funding Source не выводится автоматически из Bank Account, incoming Payment, Personal Account, Budget Item или payment method.
...
При обычной замене Meter на сохраняющейся Accounting Point старая Meter Installation заканчивается, а новая начинается. Между ними может существовать реальный период без Meter; фиктивная установка для заполнения такого gap не создаётся. Реальный overlap допускается только если он имеет самостоятельный инженерный смысл и явно объясним.

Physical mounting/removal time не обязан совпадать с effective start/end Meter Installation. При неизвестной точности времени не создаётся ложная более точная временная семантика.

Если вместе с Meter существенно изменилась сама измерительная граница, scope или функция учёта, сохранение Accounting Point не предполагается автоматически и требуется отдельное topology/accounting-point решение.
...
# 64.2. Контрольное снятие

**Контрольное снятие** (Control Observation) — исторически значимый предметный процесс организованного получения или фиксации данных по определённой области инженерной системы и временному окну с объяснимым набором ожидаемых Accounting Points и полнотой наблюдения.

Контрольное снятие не является отдельным видом Reading. Значения, признанные в его рамках, остаются обычными Reading с process context/provenance.

Для процесса могут быть существенны:

- control scope / область;
- expected Accounting Points;
- релевантная topology;
- observation window;
- participating Reading;
- missing/unresolved/rejected points;
- explicitly excluded points и основания исключения;
- acting Subject(s) / authority where applicable;
- evidence;
- completeness;
- quality limitations.

Абсолютная одновременность всех Reading не требуется; каждый Reading сохраняет собственное measurement time и temporal precision.

Контрольное снятие не тождественно Control Reconciliation или Calculated Imbalance. Оно подготавливает inputs и completeness для последующей сверки.
