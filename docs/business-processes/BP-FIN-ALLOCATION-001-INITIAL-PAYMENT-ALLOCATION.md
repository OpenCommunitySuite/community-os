...
Повторная сверка после внутреннего и независимого review подтвердила, что эти определения достаточны. `Available amount`, `Allocation Proposal` и confirmation scope остаются process semantics и не требуют добавления новых фундаментальных понятий в DOMAIN_MODEL или TERMINOLOGY.

Настоящий BP операционализирует уже принятую семантику.

`REFERENCE_CANDIDATE_MATRIX` синхронизирована:

- добавлен `REF-FIN-015 — первичное распределение признанного платежа`;
- Initial Allocation поставлен перед исправлением ошибочного recognition Payment в последовательности Этапа 5;
- конкретные Allocation Rules пилотного СТ оставлены локальной policy/configuration.
...
После принятия настоящего BP отдельная нормативная доработка DOMAIN_MODEL, TERMINOLOGY или ADR не требуется.

Следующий финансовый процесс согласно синхронизированной последовательности — `BP-FIN-002 — исправление ошибочного признания платежа`.

До перехода к нему необходимо завершить текущий банковский Этап 4 (`BP-FIN-BANK-001`) и его нормативную синхронизацию.
