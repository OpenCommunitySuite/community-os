...
**Актуально относительно:** нормативной модели после `BP-CASH-002 — Выдача наличных / исходящий наличный платёж`; DOMAIN_MODEL 0.16, TERMINOLOGY 0.14  
...
| REF-FIN-009 | Наличные приходные/расходные операции | OSBBX | Зафиксированы `BP-CASH-001` и `BP-CASH-002`: Cash Acceptance/Cash Disbursement отделены от Payment, Allocation, cash document, custody, bank deposit/withdrawal и Expense; определены amount/completion, parties/authority, offline/idempotency, 0..N Payment cardinality и incoming/outgoing bank↔cash boundaries без universal Cashbox/CashBalance/CashOperation | **Закрыт решением** | Не возвращаться к фундаментальной модели cash-channel без нового сценария; custody/internal movement вести через REF-FIN-016, подотчётные средства — REF-FIN-017 |
