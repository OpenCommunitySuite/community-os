...
| REF-FIN-009 | Наличные приходные/расходные операции | OSBBX | Наличный Payment поддержан; кассовый документ и предметный платёж различены; универсальная модель кассы не определена | **Backlog** | После основных платёжных исключений описать кассовые процессы пилотного СТ |
| REF-FIN-010 | Распределение расхода между статьями сметы | OSBBX | Expense, Budget, Financing Source, Use Direction и Expense Financing разделены; локальная семантика распределения расходов не завершена | **Backlog** | Описать BP регистрации/классификации расхода и его связи со сметой |
...
| REF-ACC-001 | Запрос на привязку пользователя к субъекту/объекту/лицевому счёту | OSBBX, МДО, DAH | ADR-010 различает Subject, User Account, Domain Power, Access Role/Right и технический доступ; конкретного процесса предоставления доступа нет | **Следующий — №1** | `BP-IAM-001`: запрос/приглашение, проверка основания, связь с субъектом, предоставление области доступа, отказ, отзыв и исправление ошибки |
| REF-IMP-001 | Первоначальный импорт объектов, субъектов, отношений, лицевых счетов и начальных данных | OSBBX, МДО | ADR-011 закрепляет migration/import, validation, mapping, preview, recognition, partial success и provenance; конкретный BP отсутствует | **Следующий — №2** | `BP-IMPORT-001`: первоначальная миграция пилотного сообщества |
| REF-SUBJ-001 | Универсальная внешняя сторона: субъект → роль → договор → операция/обязательство | OSBBX, МДО | Supplier является Subject и имеет Supplier Contract; универсальная модель договорной роли внешней стороны не принята | **Следующий — №3** | Архитектурно-предметное исследование до расширения DOMAIN_MODEL |
| REF-BANK-001 | Импорт/признание/классификация банковских сведений | OSBBX, МДО | Общая интеграционная семантика ADR-011 и Bank Transaction ADR-006 уже определены | **Следующий — №4** | `BP-FIN-BANK-001`: конкретный банковский процесс без привязки домена к одному банку |
| REF-METER-001 | Замена прибора учёта | OSBBX | ADR-007 разделяет Meter, Accounting Point и Meter Installation; история не должна разрываться при замене | **Следующий — №5** | `BP-METER-001`: снятие, конечное показание, новая установка, начальное показание, непрерывность точки учёта |
| REF-METER-002 | Автоматическое получение/импорт показаний | МДО, OSBBX | ADR-007/011: внешнее значение ≠ Reading; validation/mapping/recognition обязательны | **Backlog, высокий** | После базового BP показаний описать автоматический источник как интеграционный процесс |
| REF-METER-003 | Контрольное снятие и сверка связанных точек учёта | OSBBX, МДО | Control Reconciliation, Calculated Imbalance и Operational Loss уже определены | **Частично закрыт** | Описать практический BP контрольной сверки пилотного СТ |
| REF-OPS-001 | Обращение → операционная заявка / Work Order | DAH, частично МДО | ADR-009 определяет Appeal и прямо не делает его универсальным workflow; самостоятельная семантика операционной работы не определена | **Следующий — №6** | `BP-OPS-001`: отделить обращение от работы, результата, исполнителя, инфраструктурного объекта и затрат |
...
### Этап 5. Финансовые исключения после BP-FIN-001
...
Каждый процесс должен сохранять различия Payment, Allocation, Obligation, Accrual, Refund и Bank Transaction и не вводить универсальную Correction.
