...
| REF-ACCESS-001 | Запрос на привязку пользователя к субъекту/объекту/лицевому счёту | OSBBX, МДО, DAH | ADR-010 различает Subject, User Account, Domain Power, Access Role/Right и технический доступ; конкретного процесса предоставления доступа нет | **Следующий** | `BP-ACCESS-001`: запрос/приглашение, проверка основания, связь с субъектом, предоставление области доступа, отказ, отзыв и исправление ошибки |
| REF-IMP-001 | Первоначальный импорт объектов, субъектов, отношений и лицевых счетов | OSBBX, МДО | ADR-011 закрепляет migration/import, validation, mapping, preview, recognition, partial success и provenance; конкретный BP отсутствует | **Следующий** | `BP-IMPORT-001`: первоначальная миграция базовых предметных данных пилотного сообщества без универсального Import Workflow |
...
| REF-TRANS-001 | Финансовая прозрачность для собственников и контрольных органов | DAH | Финансовые факты и публикационная семантика существуют; конкретные состав раскрываемой информации, аудитория и правила раскрытия не определены | **Backlog** | Описать требования к проекциям, аудиториям, спорным данным и provenance |
...
| REF-AUD-001 | Контрольный доступ ревизора/контрольного органа | OSBBX, DAH | ADR-010 не требует специальной фундаментальной роли; предметные полномочия и access grants разделены | **Частично закрыт** | Реализовывать как use case общей модели доступа после BP-ACCESS-001 |
...
### Этап 1. BP-ACCESS-001 — предоставление, изменение и отзыв доступа
...
### Этап 2. BP-IMPORT-001 — миграция объектов/субъектов/отношений/лицевых счетов объектов, субъектов, отношений и лицевых счетов
...
**Результат:** `BP-IMPORT-001` без проектирования CSV/XLS-формата, таблиц и UI; отдельный backlog для миграции финансовых и ресурсных данных.
...
- банк как сторона предметного договорного/финансового отношения, если это требуется сценарием; техническая банковская интеграция остаётся внешней системой;
...
5. **BP-CASH-001 — приём наличного платежа без смешения платежа, кассового действия и кассового документа**;
6. **BP-CASH-002 — выдача наличных / исходящий наличный платёж без автоматического создания расхода сообщества**;
7. **BP-EXPENSE-001 — регистрация расхода и его связь со сметой, обязательствами, платежами и источниками финансирования**.
...
### Этап 9. Финансовая прозрачность и раскрытие информации и раскрытие финансовой информации
...
```text
1. BP-ACCESS-001 — доступ пользователя
