...
**Актуально относительно:** нормативной модели после `BP-FIN-004 — Разовое и внецикловое начисление`; DOMAIN_MODEL 0.13, TERMINOLOGY 0.11  
...
| REF-FIN-006 | Разовое/внецикловое начисление, отмена, исправление и перерасчёт | OSBBX | `BP-FIN-004` закрывает первичное one-off/out-of-cycle Accrual как process/timing semantics ordinary Accrual: target selection/resolved cases, liability, fixed amount/rules, late initial recognition, duplicate/idempotency и provenance без Accrual Batch/Snapshot/Storno; post-confirmation cancellation/correction/recalculation остаются открытыми | **Частично закрыт** | Завершить кандидата через `BP-FIN-005`: отмена/перерасчёт начисления и последствия для уже исполненных/оплаченных финансовых результатов |
...
5. **BP-FIN-005 — отмена/перерасчёт начисления и последствия для уже выполненных платежей** — следующий процесс;
