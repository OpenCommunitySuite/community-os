...
**Актуально относительно:** нормативной модели после `BP-CASH-001 — Приём наличного платежа`; DOMAIN_MODEL 0.15, TERMINOLOGY 0.13  
...
| REF-FIN-016 | Внутреннее перемещение, ответственное хранение и reconciliation наличных | пилотный СТ, BP-CASH-001 | `Cash Acceptance` фиксирует physical receipt, но полный custody/internal cash movement workflow сознательно не введён; internal transfer Community cash ≠ external Payment, universal Cashbox/CashBalance не требуется | **Backlog** | Вернуться после `BP-CASH-002` либо при появлении конкретной потребности в accountable custody, инвентаризации остатка или cash→bank reconciliation; не блокирует outgoing Cash Payment semantics |
...
| REF-FIN-009 | Наличные приходные/расходные операции | OSBBX | Входящая часть закрыта `BP-CASH-001`: Cash Acceptance отделён от Cash Payment, документа, custody и bank deposit; определены amount/change, parties/authority, offline/idempotency и cash→bank boundary без universal Cashbox/CashBalance. Исходящий наличный Payment ещё не описан | **Частично закрыт** | Завершить через `BP-CASH-002`: выдача наличных / исходящий наличный Payment без автоматического создания Expense |
...
7. **BP-CASH-002 — выдача наличных / исходящий наличный платёж без автоматического создания расхода сообщества** — следующий процесс;
8. **BP-EXPENSE-001 — регистрация расхода и его связь со сметой, обязательствами, платежами и источниками финансирования**.
