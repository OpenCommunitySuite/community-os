# Claude Independent Review — BP-FIN-BANK-001

Ты — независимый предметный и архитектурный рецензент Community OS.

Используй **только приложенные Part A, B, C и D**. Не утверждай, что видишь GitHub или другие файлы.

## Цель

Проверить специализированный процесс:
`external bank representation → Bank Transaction → classification → possible Payment/other downstream result`

## Особо проверь

1. Не смешаны ли External Representation, Bank Transaction, Payment, Allocation, Expense и Obligation.
2. Корректно ли Bank Transaction существует до classification.
3. Достаточно ли квалифицирована external transaction identity.
4. Нет ли опасной universal dedupe по date+amount/purpose.
5. Корректно ли различены redelivery / duplicate / correction / replacement / new information.
6. Не предполагает ли external correction молчаливое сохранение/смену Bank Transaction identity.
7. Корректна ли граница Bank counterparty data → Subject.
8. Не создаётся ли fake Subject / PA / Obligation для unknown данных.
9. Корректна ли связь Bank Transaction↔Payment без universal 1:1.
10. Не слишком ли широко BP включает Payment recognition.
11. Корректно ли Payment Intent используется только как matching input.
12. Корректен ли own-account transfer из двух банковских движений.
13. Не нужен ли самостоятельный domain concept Internal Transfer уже на этом этапе.
14. Корректно ли bank fee не создаёт Expense/Obligation автоматически.
15. Корректно ли "Ignored/no-action" сохраняет Bank Transaction.
16. Корректно ли разделены classification correction и external source correction.
17. Совместимы ли Retry / Reprocessing / Unknown Outcome с ADR-016.
18. Достаточно ли определена multiple-bank-account semantics.
19. Нужно ли включить Bank Account registration/lifecycle в этот BP или правильно оставить за границей.
20. Не пропущен ли важный pilot ST сценарий.
21. Требует ли BP изменений ADR/DOMAIN_MODEL/TERMINOLOGY или действующей нормы достаточно.
22. Нет ли преждевременной универсализации.
...
### A. Что сделано правильно
3–7 пунктов.

### B. Что обязательно исправить до принятия BP
Только BLOCKER/MAJOR.

### C. Нужен ли новый ADR
Да/нет + почему.

### D. Проверка own-account transfer

### E. Проверка external correction vs classification correction

### F. Проверка Payment recognition boundary

### G. Итог
Можно ли после исправлений принять BP и закрыть REF-BANK-001?

Не предлагай API/таблицы/очереди/UI и не переписывай весь BP.
