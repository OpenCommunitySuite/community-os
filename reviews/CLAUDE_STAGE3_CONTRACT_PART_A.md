# Claude Independent Review — Stage 3 / BP-CONTRACT-001

Ты — независимый предметный и архитектурный рецензент Community OS.

Используй **только приложенные Part A, Part B и Part C**. Не утверждай, что видишь GitHub или другие файлы.

## Цель

Проверить, действительно ли Community OS нужен самостоятельный `Contractual Relationship` между Community и Subject, и не создаёт ли предложенная модель:
- дублирование Subject / Supplier / Employment / Use / Representation;
- скрытый universal Contract/Party/Counterparty context;
- смешение договора как отношения, документа и финансового обязательства;
- перенос ownership понятий между bounded contexts;
- преждевременные универсальные абстракции.

## Особо проверь

1. Корректно ли, что Community не превращается в Subject.
2. Оправдана ли отдельная identity Contractual Relationship.
3. Не дублирует ли она существующее «Отношение субъекта к сообществу».
4. Правильно ли выбран owning context «Отношения субъекта с сообществом».
5. Правильно ли Supplier остаётся во владении финансового контекста.
6. Не создаёт ли модель второй identity Supplier/Counterparty.
7. Достаточно ли различены Contractual Relationship / Basis / Document / Obligation / Payment / Expense.
8. Корректна ли граница с Employment, Membership, Representation и Subject↔Object Use/Lease.
9. Корректно ли не вводить universal PartyRole / ContractItem / ContractAsset.
10. Достаточна ли бинарная baseline-модель 1 Community ↔ 1 Subject и корректно ли отложены multi-party agreements.
11. Не слишком ли свободен configurable relation kind и не превращается ли он в скрытую модель типов договора.
12. Корректна ли identity при amendment/extension/new relationship/correction.
13. Корректно ли termination не отменяет outstanding obligations.
14. Корректна ли модель ISP, который платит Community за использование опор.
15. Корректно ли различены Bank Subject / contractual bank / External Integration Party / bank counterparty data.
16. Есть ли существенный сценарий первого внедрения, который BP обязан определить до технического ТЗ, но не определяет.
17. Требует ли решение нового ADR, либо достаточно точечной синхронизации ADR-002 + DOMAIN_MODEL + TERMINOLOGY.
18. Есть ли что-то чрезмерно универсальное, что лучше удалить до нормативной синхронизации.
...
Для сомнительных выводов используй `INSUFFICIENT CONTEXT`.

После таблицы:

### A. Что в модели сделано правильно
3–7 пунктов.

### B. Что обязательно исправить до нормативной синхронизации
Только BLOCKER/MAJOR.

### C. Нужен ли новый ADR
Да/нет + аргумент.

### D. Проверка ISP scenario
Отдельно проверь аренду/использование опор и обратное финансовое направление.

### E. Проверка специализированных отношений
Трудовой договор, ГПХ, частная аренда участка, представительство.

### F. Итог
Можно ли после исправлений синхронизировать DOMAIN_MODEL / TERMINOLOGY / ADR-002?

Не переписывай весь BP и не предлагай техническую схему хранения.


---

# PART A — Полный target BP
